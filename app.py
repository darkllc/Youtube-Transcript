...
app = Flask(__name__)
CORS(app)  # Habilita CORS para todas as rota
...
def get_transcript():
    try:
        ...
        if not video_url:
            ...
            return jsonify({'error': 'URL do vídeo não fornecida.', 'status': 'error'}), 400
        ...
        if parsed_url.hostname in ['www.youtube.com', 'youtube.com']:
            ...
            if not video_id:
                ...
                return jsonify({'error': 'ID do vídeo não encontrado na URL fornecida.', 'status': 'error'}), 400
            video_id = video_id[0]
        elif parsed_url.hostname == 'youtu.be':
            video_id = parsed_url.path.lstrip('/')
        else:
            logger.warning('URL do YouTube inválida.')
            return jsonify({'error': 'URL do YouTube inválida.', 'status': 'error'}), 400
